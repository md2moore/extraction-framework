 ...
 */
class ObjectPropertyDefinition(templateContents: ListBuffer[MappingElement]) extends RootNode(templateContents) {
  override def toString(): String = {
    templateContents.mkString("\t", "\n\t", "\n")
  }

  override def getMappingSyntax(): String = {
    templateContents.mkMappingString("{{ObjectProperty\n| ", "\n| ", "\n}}")
  }
}
